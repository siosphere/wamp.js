...
class Invocation {
	constructor(procedure, progress, caller, caller_authid, caller_authrole) {
		var self = this;
...
class Event {
	constructor(publication, topic, publisher, publisher_authid, publisher_authrole) {
		var self = this;
...
export class Session {
	constructor(socket, defer, onchallenge) {
		var self = this;
		...
		self._process_INVOCATION = function (msg) {
			...
			if (registration in self._registrations) {
				...
				//
				pcall(reg.endpoint, args, kwargs, cd).then(
					function (res) {
			...
			if (!self._id) {
				...
				if (msg_type === MSG_TYPE.WELCOME) {
					...
					if (self._onchallenge) {
						...
						pcall(self._onchallenge, self, method, extra).then(
							function (signature) {
	...
	get caller_disclose_me() {
		return this._caller_disclose_me;
	}

	set caller_disclose_me(newValue) {
		this._caller_disclose_me = newValue;
	}

	get publisher_disclose_me() {
		return this._publisher_disclose_me;
	}

	set publisher_disclose_me(newValue) {
